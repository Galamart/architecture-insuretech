...
- Сервис ins-product-aggregator синхронно запрашивает данные у всех страховых компаний, что может приводить к задержкам. С ростом кол-ва компаний-партнеров эта проблема усугубится.
...
- Сервисы core-app и ins-comp-settlement обновляют свои локальные реплики с разной периодичностью => неконсистентность данных между сервисами => ошибки в работе сервисов

### 3. Увеличение нагрузки на ins-product-aggregator:
- При добавлении новых страховых компаний => рост нагрузки на ins-product-aggregator => снижение скорости отклика и рост кол-ва ошибок.
...
![EDA Scheme](InsureTech_C4_сontainer.png)